...
local function LockpickDoor(isAdvanced)
    ...
    exports['exter-lockpick']:OpenLockpickMinigame(function(success)
        if success then
            ...
            if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
                ...
                QBCore.Functions.Notify(Lang:t("message.vlockpick"), 'success')

                -- Ensure the lock is unlocked locally
                SetVehicleDoorsLocked(vehicle, 0)  -- Unlock the vehicle doors
                SetVehicleDoorsLockedForPlayer(vehicle, PlayerPedId(), false)  -- Allow player to open doors
                TriggerServerEvent('qb-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 0)  -- Unlock vehicle server-side
            end
        else
    ...
    end, vehicle, isAdvanced and 'advanced' or 'normal')  -- Passing 'advanced' or 'normal' based on isAdvanced
end
...
function lockpickFinish(success)
    ...
    if success then
        ...
        -- Debugging - Check vehicle lock status
        print("Lockpick success, attempting to unlock vehicle...")
        print("Current vehicle lock status:", GetVehicleDoorLockStatus(vehicle))

        -- Unlock vehicle doors
        SetVehicleDoorsLocked(vehicle, 0)  -- Unlock the vehicle doors
        SetVehicleDoorsLockedForPlayer(vehicle, PlayerPedId(), false)  -- Allow player to open doors

        -- Debugging - Check vehicle lock status after unlocking attempt
        print("Vehicle lock status after attempt:", GetVehicleDoorLockStatus(vehicle))
...
local function Hotwire(vehicle, plate)
    ...
    -- Start the lockpicking (hotwiring) minigame
    exports['exter-lockpick']:OpenLockpickMinigame(function(success)
        if success then
            -- Minigame succeeded, no chance needed, 100% hotwiring success
            SetVehicleAlarm(vehicle, true)
            SetVehicleAlarmTimeLeft(vehicle, hotwireTime)
            
            -- Play the animation during hotwiring
            QBCore.Functions.Progressbar("hotwire_vehicle", Lang:t("message.hotwiring"), hotwireTime, false, true, {
                ...
                -- 100% success since minigame was successful
                SetVehicleEngineOn(vehicle, true, false, true)  -- Turn on the engine
                Hotwired = plate
                IsHotwiring = false
                QBCore.Functions.Notify(Lang:t("message.hotwire_success"), "success")
            end, function() -- Cancel
                StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
                IsHotwiring = false
            end)

            -- Optional police alert after a delay
            Wait(10000)
            AttemptPoliceAlert("steal")
        else
            -- Failed minigame
    ...
    end, vehicle) -- Pass the vehicle to the minigame
end
